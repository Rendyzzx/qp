...
• *ᴇxᴘɪʀᴇᴅ:* ${(xteam.response.expired).replace('Premium left:', '')}`, m)
            ...
            case 'kingofbear':
       if (type == 'kingofbear') {
         let kingofbear = await (await fetch(`https://saipulanuar.ga/api/cekKey?apikey=${args[0]}`)).json().catch(v => 'error')
         await m.reply(cek)
         if (kingofbear == 'error') {
m.reply(`Maaf restapi ini sedang error, harap coba lagi nanti`)
} else { 
if (kingofbear.message == "Only alphanumeric!") return m.reply('Only alphanumeric!')
 if (kingofbear.message == "Apikey tidak ditemukan, silahkan daftar atau beli ke developer untuk mendapatkan apikey yang valid!") return m.reply('ɪɴᴠᴀʟɪᴅ ᴀᴘɪᴋᴇʏ !')
conn.reply(m.chat, `• *ᴛʏᴘᴇ:* KING OF BEAR
• *ᴀᴘɪᴋᴇʏ:* ${args[0]}
• *Status:* ${kingofbear.result.status}`, m)
}
...
• *ᴇxᴘɪʀᴇᴅ:* ${lol.result.expired}`, m)
            ...
            break
          default:
            return conn.sendButton(m.chat, `*${htki} CEK APIKEY ${htka}*`, 'sᴇʟᴇᴄᴛ ᴛʏᴘᴇ ᴀᴘɪᴋᴇʏ ʜᴇʀᴇ!', null, [['xᴛᴇᴀᴍ', `.cekapi ${args[0]} xteam`],['ʟᴏʟʜᴜᴍᴀɴ', `.cekapi ${args[0]} lolhuman`],['King Of Bear', `.cekapi ${args[0]} kingofbear`]],m)
        }
